...
import { Box, Button, CircularProgress, MenuItem, Modal, Paper, Select, Stack, Typography } from "@mui/material";
...
<>
  <div style={{ margin: "auto", maxWidth: "40rem" }}>
    ...
    {loading ? (
      ...
      <div style={{ padding: "5px", overflowX: "auto", backgroundColor: "#eee", maxHeight: "50vh" }}>
        {cards.map((card: any, index: number) => (
          <div
            key={index}
            style={{
          ...
          >
            ...
            <span style={{ fontSize: "12px" }}>
              <div>
                <ArvinButton onClick={() => handleSeeModal(card._id)} color="yellow">
                  <i className="fa fa-edit" aria-hidden="true" />
                </ArvinButton>
                <ArvinButton onClick={() => handleDeleteCard(card._id)} color="red">
                  <i className="fa fa-trash" aria-hidden="true" />
                </ArvinButton>
                {card.front.language && card.front.language !== "pt" && (
                  <button className="audio-button" onClick={() => readText(card.front.text, true, card.front.language)}>
                    <i className="fa fa-volume-up" aria-hidden="true" />
                  </button>
                )}
                <div style={{ fontWeight: 600 }} dangerouslySetInnerHTML={{ __html: card.front.text }} />
                <div dangerouslySetInnerHTML={{ __html: card.back.text }} />
                {card.back.language && card.back.language !== "pt" && (
                  <button className="audio-button" onClick={() => readText(card.back.text, true, card.back.language)}>
                    <i className="fa fa-volume-up" aria-hidden="true" />
                  </button>
                )}
              </div>
              <div>
                <div style={{ fontStyle: "italic" }} dangerouslySetInnerHTML={{ __html: card.backComments }} />
              </div>
            </span>
            <span>
              <ul style={{ fontSize: "10px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
                <li>Reviewed {Math.round(card.numberOfReviews)} times</li>
                <li>Review Rate Total: {card.reviewRate}</li>
                <li>Created: {card.updatedAt}</li>
                <li>
                  Tags: {card.tags.map((thetag: string, index: number) => (
  ...
  {/* Modal Overlay */}
  <div
    style={{ backgroundColor: "rgba(0,0,0,0.8)", top: 0, left: 0, display: showModal ? "block" : "none", width: "100%", height: "100%", position: "fixed" }}
    onClick={handleHideModal}
  />

  {/* Modal Content */}
  <div
    style={{ display: showModal ? "block" : "none", backgroundColor: "white", padding: "1rem", position: "fixed", top: "40%", left: "40%" }}
    id="modal"
    className="box-shadow-white"
  >
    <Xp onClick={handleHideModal}>X</Xp>
    <article id="front">
      <input style={{ maxWidth: "120px" }} value={newFront} onChange={(e) => setNewFront(e.target.value)} type="text" />
      <select style={{ maxWidth: "120px" }} value={newLGFront} onChange={(e) => setNewLGFront(e.target.value)}>
        {languages.map((language, langIndex) => (
          <option key={langIndex} value={language}>{language}</option>
        ))}
      </select>
    </article>

    <article id="back">
      <input style={{ maxWidth: "120px" }} value={newBack} onChange={(e) => setNewBack(e.target.value)} type="text" />
      <select style={{ maxWidth: "120px" }} value={newLGBack} onChange={(e) => setNewLGBack(e.target.value)}>
        {languages.map((language, langIndex) => (
          <option key={langIndex} value={language}>{language}</option>
        ))}
      </select>
      <br />
      <input style={{ maxWidth: "120px" }} value={newBackComments} onChange={(e) => setNewBackComments(e.target.value)} type="text" />
      <div>
        <ArvinButton onClick={() => handleDeleteCard(cardIdToEdit)} color="red">
          <i className="fa fa-trash" aria-hidden="true" />
        </ArvinButton>
        <ArvinButton onClick={() => handleEditCard(cardIdToEdit)} color="green">
          <i className="fa fa-folder" aria-hidden="true" />
        </ArvinButton>
      </div>
    </article>
